...
public class RumpusRestServerController extends RumpusRestController {
    ...
    public ResponseEntity<CommonSession> startServer(@PathVariable("server")
    ...
    public ResponseEntity<CommonSession> stopServer(@PathVariable("server")
    String serverName, HttpServletRequest request) {
    ...
    // return serverName + (server.getIsRunning() ? ": server is running" : ":
